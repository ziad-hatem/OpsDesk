...
5. **Configure the demo account, if used.** `POST /api/demo/reset` (guarded by `CRON_SECRET`, same shared-secret pattern as `REPORTS_SCHEDULER_SECRET`) ensures a fixed-credential demo login (`NEXT_PUBLIC_DEMO_EMAIL` / `NEXT_PUBLIC_DEMO_PASSWORD`) exists, wipes that account's organization data, and reseeds it via `scripts/tinker.mjs`'s scenarios. `vercel.json` schedules this hourly via Vercel Cron (which auto-sends `Authorization: Bearer $CRON_SECRET`) — if not deployed on Vercel, point an external scheduler at this endpoint instead. Until `NEXT_PUBLIC_DEMO_PASSWORD` and `CRON_SECRET` are set, the route returns an error and no demo account is created.
